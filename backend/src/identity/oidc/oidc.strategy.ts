...
export class OidcStrategy extends PassportStrategy(Strategy, 'oidc') {
  ...
  constructor(
    ...
    super(
      (
        ...
        Issuer.discover(`${oidcConfig.identifier}/.well-known/openid-configuration`).then((TrustIssuer) => {
  ...
  private getOIDCConfig(oidcIdentifier: string): OidcConfig {
    ...
    if (!oidcConfig) {
      ...
      throw new BadRequestException(
        `There is no ldapConfig '${oidcIdentifier}'`,
      );
    }
    return oidcConfig;
  }
  ...
  async login(oidcConfig) {
    const callbackParams = this.client.callbackParams(`/oidc/${oidcConfig.identifier}/callback`);
